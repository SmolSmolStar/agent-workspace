...
test.describe('Tasks quick launch', () => {
  test('all boards view launches using per-card board mapping', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      localStorage.setItem('tasks-board', '__all_enabled__');
      localStorage.setItem('tasks-launch-tier', '2');
    });
    ...
    await page.locator('#tasks-board').selectOption({ value: '__all_enabled__' });
  ...
  test('keyboard 1-4 launches with tier for selected card', async ({ page }) => {
    await mockUserSettings(page, {
      initial: {
        version: 'test',
        global: {
  ...
  test('all boards view card detail launch uses per-card board mapping', async ({ page }) => {
    ...
    await page.locator('.task-card-row[data-card-id="c1"]').click();
    await expect(page.locator('#tasks-launch-btn')).toBeVisible({ timeout: 20000 });
    await page.locator('#tasks-launch-btn').click();
  ...
  test('list view tier buttons launch with selected tier', async ({ page }) => {
    await mockUserSettings(page, {
      initial: {
        version: 'test',
        global: {
    ...
    // Open Tasks in list view for determinism.
    await page.evaluate(() => {
      localStorage.setItem('tasks-view', 'list');
      localStorage.setItem('tasks-board', 'b1');
    ...
    await page.locator('.task-card-row[data-card-id="c1"] [data-quick-launch-tier-btn="1"]').click();
    await page.waitForFunction(() => (window.__launchCalls?.length || 0) > 0, null, { timeout: 10000 });
    const calls = await page.evaluate(() => window.__launchCalls || []);
    const last = calls[calls.length - 1];
    expect(last.tier).toBe(1);
